...
#define MAXLEFTSLIDEWIDTH (WIDTH_SCREEN/6)
...
static bool const isMoveable = false;
...
- (void)prepare{
    self.leftMenuViewController.view.frame = CGRectMake(0, 0, MAXLEFTSLIDEWIDTH*2, HEIGHT_SCREEN);
...
//    _gestureRecognizerView.hidden = YES;
    ...
    _gestureRecognizerView.alpha = 0;
    ...
    _contentViewVisibleWidth = MAXLEFTSLIDEWIDTH;
...
- (void)panGestureRecognizer:(UIPanGestureRecognizer *)recognizer{
    if(isMoveable){
        self.gestureRecognizerView.hidden = NO;
        ...
        _gestureRecognizerView.alpha = _contentViewContainer.lx_x /(MAXLEFTSLIDEWIDTH*2);
        NSLog(@"_menuViewContainer%f",_menuViewContainer.lx_x);
        if (_gestureRecognizerView.alpha > 0.65) {
            _gestureRecognizerView.alpha = 0.65;
        }
    }
    
}
- (void)showMenu:(BOOL)show{
    _gestureRecognizerView.backgroundColor = [UIColor blackColor];
    _gestureRecognizerView.alpha = 0.65;
    if (_scaleContent) {
...
-(UIScreenEdgePanGestureRecognizer *)pan1{
    if (!_pan1) {
        _pan1 =[[UIScreenEdgePanGestureRecognizer alloc]initWithTarget:self action:@selector(panGestureRecognizer:)];
