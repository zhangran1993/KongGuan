...
- (void)addAttribute:(CCZTrotingAttribute *)attribute {
    ...
    self.label.frame = CGRectMake(40, 0, textSize.width, textSize.height);
...
-(void)labelTouchUpInside:(UITapGestureRecognizer *)recognizer{
    if ([self.delegate respondsToSelector:@selector(CCZTrotingLabelClick:index:)]) {
        [self.delegate CCZTrotingLabelClick:self.label.text index:self.index];
    }
}
...
- (void)addTrotingAttributes:(NSArray<CCZTrotingAttribute *> *)atts {
    ...
    self.attributeArrs = [atts mutableCopy];
