...
- (void)calculate {
    switch (self.type) {
        ...
        case CCZWalkerTypeDescend: {
            // 设置初始位置
            self.frameBegin = CGRectMake(0, self.wSize.height*2, self.wSize.width, self.wSize.height);
            // 设置下滚显示位置
            self.frame1 = CGRectMake(0, (self.aSize.height - self.wSize.height) / 2, self.wSize.width, self.wSize.height);
            // 设置在长度显示不下时，左滚一段距离的位置
            if (self.wSize.width > CGRectGetWidth(self.contentView.frame)) {
                self.frame2 = CGRectMake(-(self.wSize.width - CGRectGetWidth(self.contentView.frame)) - FrameWidth(50), CGRectGetMinY(self.frame1), self.wSize.width, self.wSize.height);
            } else {
    ...
    switch (self.type) {
            case CCZWalkerTypeDefault: {
                ...
                [UIView animateWithDuration:self.duration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                    ...
                    [UIView animateWithDuration:self.wSize.width / velocity delay:3 options:UIViewAnimationOptionCurveLinear animations:^{
                ...
                [UIView animateWithDuration:self.duration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                    ...
                    if (self.wSize.width > CGRectGetWidth(self.contentView.frame)) {
                        [UIView animateWithDuration:(self.frame1.size.width - CGRectGetWidth(self.contentView.frame)) / velocity delay:self.pause options:UIViewAnimationOptionCurveLinear animations:^{
                            ...
                            [UIView animateWithDuration:self.duration delay:3 options:UIViewAnimationOptionCurveLinear animations:^{
                                self.walkerView.frame = self.frameEnd;
                            } completion:^(BOOL finished) {
                                self.walkerView.frame = self.frameBegin;
                                self.isWalking = NO;
                        ...
                        [UIView animateWithDuration:self.duration delay:3 options:UIViewAnimationOptionCurveLinear animations:^{
                            ...
                            [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(waiting) userInfo:nil repeats:NO];
                           // self.walkerView.frame = self.frameBegin;
                            self.isWalking = NO;
