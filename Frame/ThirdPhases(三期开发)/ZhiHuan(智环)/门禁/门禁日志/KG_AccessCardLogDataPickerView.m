...
@interface KG_AccessCardLogDataPickerView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
/** 时间选择器类型 */
@property(nonatomic , assign) AccessDatePickerType datePickerType;

@end


@implementation KG_AccessCardLogDataPickerView

#pragma mark - init
/// 初始化
/** 构造方法 */
- (instancetype)initWithFrame:(CGRect)frame withDatePickerType:(AccessDatePickerType)datePickerType{
...
-(void)creatDatePickerWithWYLDatePickerType:(AccessDatePickerType)datePickerType{
    switch (datePickerType) {
            
        case AccessDatePickerTypeYMDHMS:
        {
            ...
            break;
        case AccessDatePickerTypeYMDHM:
        {
            self.minuteArr = [[NSMutableArray alloc]init];
            [self.dataArray addObject:self.yearArr];
            [self.dataArray addObject:self.monthArr];
        ...
        case AccessDatePickerTypeYM:
        {
            [self.dataArray addObject:self.yearArr];
            [self.dataArray addObject:self.monthArr];
        }
            break;
        case AccessDatePickerTypeY:
        {
            [self.dataArray addObject:self.yearArr];
        }
            break;
...
- (void) configDataWithWYLDatePickerType:(AccessDatePickerType)datePickerType {
...
- (void)setMinuteInterval:(NSInteger)minuteInterval {
    ...
    if (self.datePickerType == AccessDatePickerTypeYMDHM) {
    ...
    }else if(self.datePickerType == AccessDatePickerTypeYMDHMS){
        if (self.minuteArr.count > 0) {
            [self.minuteArr removeAllObjects];
            self.minuteArr = [self configMinuteArray];
            [self.dataArray replaceObjectAtIndex:self.dataArray.count - 1 withObject:self.minuteArr];
...
- (void)show {
    ...
    if (self.datePickerType != AccessDatePickerTypeY) {
    ...
    switch (self.datePickerType) {
        ...
        case AccessDatePickerTypeYMDHMS:
        {
            ...
            self.second = [NSString stringWithFormat:@"%ld秒", [self.timeArr[5] integerValue]];
            ...
            [self.pickerView selectRow:[self.secondArr indexOfObject:self.second] inComponent:5 animated:YES];
            /// 刷新日
            [self refreshDay];
        }
            break;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    }else if (self.datePickerType == AccessDatePickerTypeYMD){
    ...
    }else if (self.datePickerType == AccessDatePickerTypeYM){
    ...
    } else if (self.datePickerType == AccessDatePickerTypeY) {
        switch (component) {
            case 0: { // 年
