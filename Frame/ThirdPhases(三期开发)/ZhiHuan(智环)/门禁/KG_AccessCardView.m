...
#define nvbH 40
@interface KG_AccessCardView ()<SegmentTapViewDelegate> {
...
@property (nonatomic, strong) KG_AccessCardCeDianViewController *cedianView;
@property (nonatomic, strong) KG_AccessCardControlViewController *controlView;
@end

@implementation KG_AccessCardView
...
- (void)createSegmentView{
    ...
    //把页面添加到scroView的三个页面
    self.cedianView = [[KG_AccessCardCeDianViewController alloc]init];
    ...
    self.controlView = [[KG_AccessCardControlViewController alloc]init];
    self.controlView.controlLog = ^(NSDictionary * _Nonnull dataDic) {
     
        [scrollView setContentOffset:CGPointMake(SCREEN_WIDTH *self.currIndex, 0)];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"controlAccessLog" object:dataDic];
