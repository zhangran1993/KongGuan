...
#import "KG_KongTiaoDetailViewController.h"
...
@interface KG_KongTiaoViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation KG_KongTiaoViewController
...
//    NSString *  FrameRequestURL  =  [NSString stringWithFormat:@"http://10.33.33.147:8089/intelligent/api/envRoomInfo/%@/%@",_station_code,@""];
    NSString *  FrameRequestURL = [WebNewHost stringByAppendingString:[NSString stringWithFormat:@"/intelligent/api/envRoomInfo/%@/%@",_station_code,@""]];
    [FrameBaseRequest getWithUrl:FrameRequestURL param:nil success:^(id result) {
        NSInteger code = [[result objectForKey:@"errCode"] intValue];
        if(code  <= -1){
            [FrameBaseRequest showMessage:result[@"errMsg"]];
...
//    NSString *  FrameRequestURL = [NSString stringWithFormat:@"http://10.33.33.147:8089/intelligent/api/envDeviceInfo/%@/%@",_station_code,_category];
    ...
    [FrameBaseRequest getWithUrl:FrameRequestURL param:nil success:^(id result) {
        ...
        for (int i = 0; i < equipmentDetails.count; i++) {
            ...
            KG_KongTiaoDetailViewController *oneVC = [[KG_KongTiaoDetailViewController alloc] init];
            oneVC.moreAction = ^{
