...
#import "KG_ContactTopThirdView.h"
@interface KG_AddressbookThirdViewController ()
...
@implementation KG_AddressbookThirdViewController{
...
- (void)createNaviTopView {
    ...
    self.titleLabel.text = safeString(self.thirdTitle);
...
- (void)sureMethod {
    ...
    if (vcArr.count >3) {
        [self.navigationController popToViewController:vcArr[vcArr.count -4] animated:YES];
    }
...
- (void)queryContactData {
    ...
    params[@"type"] = @"eq";
    params[@"content"] = safeString(self.thirdID);
...
- (void)refreshPageView {
    ...
    for (orgInfo *info in self.dataModel.orgInfo) {
        secHeight += info.subOrgInfo.count *40;
    }
    UIView *headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, height + secHeight +40 +46)];
    KG_ContactTopThirdView *topView = [[KG_ContactTopThirdView alloc]init];
    topView.secTitle = safeString(self.secondTitle);
    topView.thirdTitle = safeString(self.thirdTitle);
    ...
    topView.searchMethod = ^(NSString * _Nonnull searchStr) {
      ...
      vc.searchId = safeString(self.thirdID);
