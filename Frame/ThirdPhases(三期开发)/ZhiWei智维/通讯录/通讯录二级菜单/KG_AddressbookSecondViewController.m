...
#import "KG_AddressbookThirdViewController.h"
...
@interface KG_AddressbookSecondViewController ()
...
@property (nonatomic, strong)  KG_AddressbookSecondModel *dataModel;
...
@implementation KG_AddressbookSecondViewController{
...
- (void)viewDidLoad {
    ...
    self.dataModel = [[KG_AddressbookSecondModel alloc]init];
...
- (void)createNaviTopView {
    ...
    self.titleLabel.text = safeString(self.secondTitle);
...
- (void)sureMethod {
    ...
    [dataDic setValue:safeString(self.nameStr) forKey:@"nameStr"];
    [dataDic setValue:safeString(self.nameID) forKey:@"nameID"];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"addressBookSelPerson"
      object:self
    userInfo:dataDic];
...
//        if ([VC isKindOfClass:[KG_CreateXunShiContentViewController class]] ||
//            [VC isKindOfClass:[KG_CreateDayWeiHuViewController class]] ||
//            [VC isKindOfClass:[KG_CreateMonthWeiHuViewController class]] ||
//            [VC isKindOfClass:[KG_CreateYearWeiHuViewController class]] ||
//            [VC isKindOfClass:[KG_CreateTeShuBaoZhangViewController class]] ){
//
...
- (void)queryContactData {
    ...
    params[@"type"] = @"eq";
    params[@"content"] = safeString(self.strID);
    ...
    [FrameBaseRequest postWithUrl:FrameRequestURL param:paraArr success:^(id result) {
        ...
        for (int i =0; i<self.dataModel.contacts.count; i++) {
            seccontacts *contact = self.dataModel.contacts[i];
...
- (void)refreshPageView {
    float height = self.dataModel.orgInfo.count *44;
    float secHeight = 0;
    
    UIView *headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, height + secHeight +40+56)];
    KG_ContactTopSecondView *topView = [[KG_ContactTopSecondView alloc]init];
    topView.model = self.dataModel;
    topView.secTitle = self.secondTitle;
    topView.pushToNextPage = ^(NSInteger selSection, NSInteger selIndex) {
        KG_AddressbookThirdViewController *vc = [[KG_AddressbookThirdViewController alloc]init];
        NSArray *arr = self.dataModel.orgInfo;
        if (arr.count) {
            secorgInfo *dataDic = arr[selIndex];
            vc.secondTitle = safeString(self.secondTitle);
            vc.thirdTitle = safeString(dataDic.orgName);
            vc.thirdID = safeString(dataDic.orgId);
        }
    ...
    topView.searchMethod = ^(NSString * _Nonnull searchStr) {
        ...
        vc.searchId = safeString(self.strID);
