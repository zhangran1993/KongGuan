...
@property (nonatomic, strong)   KG_CreateYearWeiHuModel *dataModel;
...
@implementation KG_CreateYearWeiHuViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.title = @"新建季/年维护";
    self.dataModel = [[KG_CreateYearWeiHuModel alloc]init];
    self.dataModel.taskType = @"quarterSafeguard";//默认周维护
...
- (void)reportMethod:(UIButton *)button {
    ...
    if (self.dataModel.selStartTime.length == 0 ||
        self.dataModel.selEndTime.length == 0) {
    ...
    NSString *ss = [self nsstringConversionNSDate:safeString(self.dataModel.selStartTime)];
    if (ss.length >0) {
        params[@"planStartTime"] = safeString(ss);
    }
    NSString *ssend = [self nsstringConversionNSDate:safeString(self.dataModel.selEndTime)];
    if (ssend.length >0) {
        params[@"planFinishTime"] = safeString(ssend);
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    KG_CreateYearWeiHuCell *cell = [tableView dequeueReusableCellWithIdentifier:@"KG_CreateYearWeiHuCell"];
    if (cell == nil) {
        cell = [[KG_CreateYearWeiHuCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"KG_CreateYearWeiHuCell"];
    }
    if (self.dataArray.count >0) {
        if (indexPath.section == 3) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (self.dataArray.count == 0) {
        ...
        if (section == 0) {
            ...
            [self.normalBtn setTitle:@"年维护" forState:UIControlStateNormal];
            self.normalBtn.titleLabel.font = [UIFont systemFontOfSize:14];
            [self.normalBtn addTarget:self action:@selector(setTaskType:) forControlEvents:UIControlEventTouchUpInside];
            ...
            [self.yijianBtn setTitle:@"季维护" forState:UIControlStateNormal];
            self.yijianBtn.titleLabel.font = [UIFont systemFontOfSize:14];
            [self.yijianBtn addTarget:self action:@selector(setTaskType:) forControlEvents:UIControlEventTouchUpInside];
            ...
            if ([self.dataModel.taskType isEqualToString:@"quarterSafeguard"]) {
        ...
        if (section == 0) {
            ...
            titleLabel.text = safeString(@"时间范围");
           
            self.startButton = [[UIButton alloc]init];
            [headView addSubview:self.startButton];
            [self.startButton setTitle:@"开始日期" forState:UIControlStateNormal];
            if(safeString(self.dataModel.selStartTime).length >0) {
                
                NSString *ss = [safeString(self.dataModel.selStartTime) substringToIndex:10];
                [self.startButton setTitle:safeString(ss) forState:UIControlStateNormal];
            ...
            [headView addSubview:self.endButton];
            ...
            if(safeString(self.dataModel.selEndTime).length >0) {
                
                NSString *ss = [safeString(self.dataModel.selEndTime) substringToIndex:10];
                [self.endButton setTitle:safeString(ss) forState:UIControlStateNormal];
...
- (void)setTaskType:(UIButton *)button {
    ...
    if (i == 1) {
        self.dataModel.taskType = @"quarterSafeguard";//默认季维护
        self.normalBtn.layer.borderColor = [[UIColor colorWithHexString:@"#F1F1F1"] CGColor];
        self.normalBtn.layer.borderWidth = 1.f;
        self.normalBtn.layer.cornerRadius = 4.f;
        self.normalBtn.layer.masksToBounds = YES;
        ...
        self.dataModel.taskType = @"yearSafeguard";//年维护
...
- (void)quertFrameData{
    ...
    NSString *  FrameRequestURL = [WebNewHost stringByAppendingString:[NSString stringWithFormat:@"/intelligent/atcSafeguard/templateList/%@/%@/%@",safeString(stationCode),@"routineMaintenance",safeString(self.dataModel.taskType)]];
...
- (void)datePickerViewSaveBtnClickDelegate:(NSString *)timer {
    ...
    if (self.currIndex == 0) {
        self.startTime = timer;
        self.dataModel.selStartTime = timer;
        
        if(timer.length >10) {
            NSString *ss = [timer substringToIndex:10];
            [self.startButton setTitle:safeString(ss) forState:UIControlStateNormal];
        }
    }else {
        self.endTime = timer;
        self.dataModel.selEndTime = timer;
        if(timer.length >10) {
            NSString *ss = [timer substringToIndex:10];
            [self.endButton setTitle:safeString(ss) forState:UIControlStateNormal];
        }
