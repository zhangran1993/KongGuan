...
@implementation KG_CreateTeShuBaoZhangViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.title = @"新建特殊保障";
    self.dataModel = [[KG_CreateTeShuBaoZhangModel alloc]init];
...
- (void)reportMethod:(UIButton *)button {
    ...
    params[@"typeCode"] = @"specialTour";
    params[@"patrolCode"] = safeString(self.dataModel.reportTaskTypeCode);
    params[@"patrolId"] = safeString(self.dataModel.xunshiRoomID);
    params[@"taskName"] = safeString(self.dataModel.taskTitle);
    
    NSString *FrameRequestURL = [WebNewHost stringByAppendingString:[NSString stringWithFormat:@"/intelligent/atcSafeguard/addSpecialSecurity"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    KG_CreateTeShuBaoZhangCell *cell = [tableView dequeueReusableCellWithIdentifier:@"KG_CreateTeShuBaoZhangCell"];
    if (cell == nil) {
        cell = [[KG_CreateTeShuBaoZhangCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"KG_CreateTeShuBaoZhangCell"];
    }
    if (self.dataArray.count >0) {
        if (indexPath.section == 2) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (self.dataArray.count == 0) {
        ...
        if (section == 0) {
            ...
            titleLabel.text = safeString(@"所属台站");
        ...
        if (section == 0) {
            ...
            self.taskTypeButton = [[UIButton alloc]init];
            [headView addSubview:self.taskTypeButton];
            
            self.taskTypeButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
            [self.taskTypeButton setTitle:@"请选择任务类型" forState:UIControlStateNormal];
            if(self.dataModel.reportTaskTypeName.length >0) {
                [self.taskTypeButton setTitle:safeString(self.dataModel.reportTaskTypeName) forState:UIControlStateNormal];
            }
            [self.taskTypeButton setTitleColor:[UIColor colorWithHexString:@"#BABCC4"] forState:UIControlStateNormal];
            self.taskTypeButton.titleLabel.font = [UIFont systemFontOfSize:14];
            [self.taskTypeButton addTarget:self action:@selector(selectTaskTypeMethod:) forControlEvents:UIControlEventTouchUpInside];
            [self.taskTypeButton mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerY.equalTo(headView.mas_centerY);
                make.right.equalTo(rightImage.mas_left).offset(-1);
                make.height.equalTo(headView.mas_height);
                make.width.equalTo(@150);
...
//选择任务类型
- (void)selectTaskTypeMethod:(UIButton *)button {
    if (_taskTypeView == nil) {
        [JSHmainWindow addSubview:self.taskTypeView];
        self.taskTypeView.didSelBlock = ^(NSDictionary * _Nonnull dataDic) {
            self.dataModel.reportTaskTypeName = safeString(dataDic[@"name"]);
            self.dataModel.reportTaskTypeCode = safeString(dataDic[@"code"]);
            
            [self.tableView reloadData];
        };
        [self.taskTypeView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        self.taskTypeView.hidden = NO;
    }
    
}

- (KG_SelTaskTypeView *)taskTypeView {
    
    if (!_taskTypeView) {
        _taskTypeView = [[KG_SelTaskTypeView alloc]init];
    }
