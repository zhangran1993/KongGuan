...
@property (nonatomic ,strong) ZRBarChart   *chartBar;
...
- (void)createSubviewsView {
    ...
    self.chartBar = [[ZRBarChart alloc] initWithFrame:CGRectMake(16, 50, SCREEN_WIDTH -32, 250) withMarkLabelCount:6 withOrientationType:OrientationVertical];
...
//        for (NSDictionary *dataDic in listArray) {
//            [numArray addObject:safeString(dataDic[@"num"])];
//        }
//        self.chartBar .valueArray = numArray;
//
//        NSMutableArray *colorArray = [NSMutableArray arrayWithCapacity:0];
//        for (NSDictionary *dataDic in self.listArray) {
//            BOOL inUse = dataDic[@"inUse"];
//            if (inUse) {
...
- (void)setDataDic:(NSDictionary *)dataDic {
    ...
    if (dataDic.count >0 && !self.isShow) {
        
        self.isShow = YES;
        NSArray *titleArray = dataDic[@"xArr"];
        ...
        for (int i = 0;i< titleArray.count; i++) {
            ...
            NSArray *color = @[[self colorWithHexString:@"#FC1F42" alpha:1],[self colorWithHexString:@"#FC1F42" alpha:1]];
            [colorArray addObject:color];
