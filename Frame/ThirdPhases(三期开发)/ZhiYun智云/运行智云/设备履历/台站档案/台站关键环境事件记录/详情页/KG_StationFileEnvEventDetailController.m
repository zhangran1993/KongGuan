...
@interface KG_StationFileEnvEventDetailController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation KG_StationFileEnvEventDetailController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    NSArray *arr = [NSArray arrayWithObjects:@"事件标题",@"事件时间",@"事件类型",@"具体类型",@"事件状态",@"详细描述", nil];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 5) {
        CGRect fontRect = [safeString(self.dataDic[@"recordDescription"]) boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 100, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObject:[UIFont systemFontOfSize:14] forKey:NSFontAttributeName] context:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 5) {
        ...
        if (indexPath.row == 5) {
            cell.detailLabel.text = safeString(self.dataDic[@"recordDescription"]);
        }
    ...
    if (indexPath.row == 0) {
        cell.detailLabel.text = safeString(self.dataDic[@"name"]);
