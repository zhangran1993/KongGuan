...
#define navHeight self.navigationHeight * 1
#define navHeight2 self.navigationHeight * 1
#import "ZYSpreadButton.h"

@interface ZYSpreadButton () <CAAnimationDelegate>
@property (strong, nonatomic) UIDynamicAnimator *animator;
@property (nonatomic, strong) NSString *btnPosition;
@end

@implementation ZYSpreadButton

- (instancetype)initWithBackgroundImage:(UIImage *)backgroundImage highlightImage:(UIImage *)highlightImage position:(CGPoint)position navigationHeight:(CGFloat)navigationHeight{
    ...
    if (self) {
        ...
        //main Button
        self.powerButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 65, 65)];
...
/*临时添加
 *
 if (location.y <  navHeight) {
 location.y = navHeight;
 }
 destinationLocation = CGPointMake(location.x+_touchBorderMargin*3, self.bounds.size.width/2 + _touchBorderMargin);
 */
            ...
            switch (_positionMode) {
                ...
                case SpreadPositionModeTouchBorder: {
                    ...
                    if (location.y < magneticDistance) {//上面区域
                        if (location.x < superViewSize.width/2) {
                            destinationLocation = CGPointMake(location.x+_touchBorderMargin*3, self.bounds.size.width/2 + _touchBorderMargin);
                            /*
                            if (location.y <  navHeight) {
                                location.y = navHeight;
                            }
                            destinationLocation = CGPointMake(self.bounds.size.width/2 + _touchBorderMargin, location.y + _touchBorderMargin*2);
                             */
                        } else {
                            destinationLocation = CGPointMake(location.x-_touchBorderMargin*3, self.bounds.size.width/2 + _touchBorderMargin);
                            /*
                            ...
                            destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin), location.y + _touchBorderMargin*2);
                             */
                        }
                    } else if (location.y > superViewSize.height - magneticDistance) {//下面
                        if (location.x < superViewSize.width/2) {
                             destinationLocation = CGPointMake(location.x+_touchBorderMargin*3, superViewSize.height - self.bounds.size.height/2 - _touchBorderMargin);
                            /*
                            if (location.y > HEIGHT_SCREEN - navHeight2) {
                                location.y = HEIGHT_SCREEN - navHeight2;
                            }
                            destinationLocation = CGPointMake(self.bounds.size.width/2 + _touchBorderMargin, location.y - _touchBorderMargin*2);
                             */
                        } else {
...
//                            destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin), location.y - _touchBorderMargin*2);
                        ...
                        destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin), location.y-_touchBorderMargin);
                    } else {//左边
                        if (location.y > HEIGHT_SCREEN - navHeight2) {
                            location.y = HEIGHT_SCREEN - navHeight2;
                        }
                        destinationLocation = CGPointMake(self.bounds.size.width/2 + _touchBorderMargin, location.y-_touchBorderMargin);
                    }
...
- (void)spreadSubButton {
    ...
    //为了防止按钮移到在屏幕左侧时  按钮排序问题  reversedArray
    NSArray* reversedArray = [NSArray new];
    if (_direction == SpreadDirectionRight) {
        reversedArray = [[_subButtons reverseObjectEnumerator] allObjects];
    } else {
        reversedArray =_subButtons;
    }
    for (ZYSpreadSubButton *btn in reversedArray) {
...
- (CGPoint)calculatePointWithAngle:(CGFloat)angle radius:(CGFloat)radius {
    ...
    if ([self.btnPosition isEqualToString:@"Top"]) {
        return CGPointMake(x, y+50);
    } else if([self.btnPosition isEqualToString:@"Bottom"]){
        return CGPointMake(x, y-50);
    } else if([self.btnPosition isEqualToString:@"Left"]){
        return CGPointMake(x+50, y);
    } else if([self.btnPosition isEqualToString:@"Right"]){
        return CGPointMake(x-50, y);
    }
...
- (void)setSubButtons:(NSMutableArray *)subButtons {
