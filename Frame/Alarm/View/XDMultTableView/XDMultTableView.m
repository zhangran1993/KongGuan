...
@interface XDMultTableView()<EmptyDataSetDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame{
    if ([super initWithFrame:frame]) {
        ...
        _multopenSectionArray = [arr mutableCopy];
        _tableView = [[UITableView alloc] initWithFrame:self.bounds];
        _tableView.delegate = self;
        _tableView.dataSource = self;
        _tableView.emptyDataSetView.hidden = YES;
...
- (void)setupRefresh{
    ...
    MJRefreshNormalHeader *header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        if ([weakSelf.rollDelegate respondsToSelector:@selector(tableViewRequestData)]) {
            [weakSelf.rollDelegate tableViewRequestData];
        }
    }];
...
- (void)setOpenSectionArray:(NSArray *)openSectionArray {
    _multopenSectionArray = [openSectionArray mutableCopy];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if ([self.rollDelegate respondsToSelector:@selector(startRoll)]) {
        [self.rollDelegate startRoll];
    }
}
